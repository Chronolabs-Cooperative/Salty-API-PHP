...
define("API_FILE_IO_FOOTER", dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR . 'api-%s.html');
define("API_PATH_IO_FUNCTIONS", __DIR__ . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'functions');
...
if (!function_exists("getPingTiming")) {
	...
	/**
	 * getPingTiming ~ gets ping time of peered API services
	 * 
	 * @param string $path
	 * @param string $pinguri
	 * @return float
	 */
	function getPingTiming($path = '', $pinguri = '')
	{
		if (!$pings = readCache("ping-peers"))
			$pings = array();
		if ($pings[$path]['next']<time())
			unset($pings[$path]);
		if (!isset($pings[$path]['ms']))
		{
			return round(getURIData($pinguri, array('uri'=>(isset($_SERVER["HTTPS"])?"https://":"http://") . strtolower($_SERVER["HTTP_HOST"]) . API_URL_BASE_PATH . "pong.api",
										'time'=>microtime(true),
										'zone'=>ini_get('date.timezone'),
										'from'=>(isset($_SERVER["HTTPS"])?"https://":"http://") . strtolower($_SERVER["HTTP_HOST"]) . API_URL_BASE_PATH )),3);
		} else {
			return round($pings[$path]['ms'],3);
		}
	}
}
...
if (!function_exists("getPeersSupporting")) {
	...
	/**
	 * getPeersSupporting() ~ gets API Peers Supporting this API
	 */
	function getPeersSupporting()
	{
		...
		if (empty($ret))
		{
			...
			foreach($peerings as $peer)
			{
				...
				$ret[$realm]['ping']['uri'] = $parts[3];
				$ret[$realm]['path'] = $parts[4];
				$ret[$realm]['uri'] = $parts[5];
				$ret[$realm]['contact'] = $parts[6];
				$ret[$realm]['ping'] = getPingTiming($parts[4], $parts[1].$parts[3]);
			}
		}
		return $ret;
	}
